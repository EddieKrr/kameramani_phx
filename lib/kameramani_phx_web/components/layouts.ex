defmodule KameramaniPhxWeb.Layouts do
  ...
  def app(assigns) do
    ...
    ~H"""
    <header class={"navbar fixed top-0 left-0 right-0 z-50 flex items-center justify-between bg-slate-800/60 backdrop-blur-md px-4 sm:px-6 lg:px-8 #{if !is_chat_page, do: "mx-auto max-w-[1440px]"}"}>
      <div class={"mx-4 flex items-center gap-4 justify-between w-full px-4 sm:px-6 py-2 #{if !is_chat_page, do: "rounded-xl bg-slate-800/60 backdrop-blur-sm border-2 mt-3 border-slate-700"}"}>
        <div class="flex">
          <.link
            href="/"
