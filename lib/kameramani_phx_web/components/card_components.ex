defmodule KameramaniPhxWeb.CardComponents do
  ...
  def card(assigns) do
    ~H"""
    <div class="article">
      <div class="thmb relative aspect-video w-full bg-black">
        <div class="lv-ind absolute top-1 left-1 bg-red-500 rounded-full px-1 text-xs">LIVE</div>
        <div class="v-cnt absolute bottom-1 right-1 bg-green-300/50 rounded-full text-xs px-1">3000</div>
      </div>
      <div class="m-dt flex flex row gap-1">
      ...
      <div class="tag bg-gray-700 text-xs text-gray-300 rounded-full px-2">Just Chatting</div>
    </div>
    """
  end
end
