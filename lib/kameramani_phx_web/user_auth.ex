defmodule KameramaniPhxWeb.UserAuth do
  use KameramaniPhxWeb, :verified_routes
  ...
  @remember_me_cookie "_kameramani_phx_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      KameramaniPhxWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      KameramaniPhxWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  Or use the `live_session` of your router to invoke the on_mount callback:
      ...
      live_session :authenticated, on_mount: [{KameramaniPhxWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount({:require_role, role_name}, _params, session, socket) do
    socket = mount_current_user(socket, session)
    user = socket.assigns.current_user.user

    if user && Accounts.user_has_role?(user, role_name) do
      {:cont, socket}
    else
      socket =
        socket
